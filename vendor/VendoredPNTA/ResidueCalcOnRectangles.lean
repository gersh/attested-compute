...
Modified by Gershon Bialer, 2026, for Lean/mathlib compatibility.
-/
import Architect
...
lemma residue_analyticAt_eq_zero {f : ℂ → ℂ} {p : ℂ} (hf : AnalyticAt ℂ f p) :
  ...
  have hsub : Filter.Tendsto (fun z : ℂ ↦ z - p) (nhdsWithin p {p}ᶜ) (nhds 0) := by
    have hfun : ((id : ℂ → ℂ) - fun _ : ℂ ↦ p) = fun z ↦ z - p := by
      funext z
      rfl
    rw [← hfun]
    have hcont : Continuous ((id : ℂ → ℂ) - fun _ : ℂ ↦ p) :=
      continuous_id.sub continuous_const
    have hzero : (((id : ℂ → ℂ) - fun _ : ℂ ↦ p) p) = 0 := by
      rw [hfun]
      simp
    have ht : Filter.Tendsto ((id : ℂ → ℂ) - fun _ : ℂ ↦ p)
        (nhdsWithin p {p}ᶜ)
        (nhds (((id : ℂ → ℂ) - fun _ : ℂ ↦ p) p)) :=
...
lemma residue_eq_zero_of_not_pole_of_meromorphicAt {F : ℂ → ℂ} {s : ℂ}
  ...
  have hsub : Filter.Tendsto (fun z : ℂ ↦ z - s) (nhdsWithin s {s}ᶜ) (nhds 0) := by
    have hfun : ((id : ℂ → ℂ) - fun _ : ℂ ↦ s) = fun z ↦ z - s := by
      funext z
      rfl
    rw [← hfun]
    have hcont : Continuous ((id : ℂ → ℂ) - fun _ : ℂ ↦ s) :=
      continuous_id.sub continuous_const
    have hzero : (((id : ℂ → ℂ) - fun _ : ℂ ↦ s) s) = 0 := by
      rw [hfun]
      simp
    have ht : Filter.Tendsto ((id : ℂ → ℂ) - fun _ : ℂ ↦ s)
        (nhdsWithin s {s}ᶜ)
        (nhds (((id : ℂ → ℂ) - fun _ : ℂ ↦ s) s)) :=
      hcont.continuousAt.continuousWithinAt.tendsto
    rwa [hzero] at ht
