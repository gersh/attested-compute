...
private lemma tendsto_mul_self_of_sub_principal_isBigO_one
  ...
  have hp_tendsto :
    ...
    -- LOCAL ADAPTATION (gpu_prover vendoring): the original used `simpa`, which
    -- under this Mathlib pin rewrites `fun z => z - p` into `id - fun _ => p`
    -- and no longer matches.  Avoid `simp` on the function itself.
    have hc : Tendsto (fun z : ℂ => z - p) (nhds p) (nhds (p - p)) :=
      (continuous_sub_right p).tendsto p
    rw [sub_self] at hc
