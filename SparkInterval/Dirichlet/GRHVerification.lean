...
every primitive character of the modulus has all its nontrivial L-function
zeros in the checked source strip on the critical line.
...
/-- Finite GRH verification statement for one modulus and one source strip:
every nontrivial zero of every primitive-character L-function in
