...
"""In-CVM prelude for the CH25 Lemma A.7 Phala/dstack Intel TDX campaign.
...
NO TDX HARDWARE WAS AVAILABLE when this script was written.  Its guest-agent
interaction is exercised only against an in-process mock in
``tests/test_phala_tdx_prelude.py``.  The real path has never been executed.
"""
...
# Exactly `RETAINED_ARTIFACT_SHA256` in tools/tg_a7_phala_tdx_workload.py and
# `retained_artifact_sha256` in the registered input below.  The producer
# re-checks it; this is the earlier of the two checks, not a replacement.
RETAINED_ARTIFACT_SHA256 = (
    "ccc11cecdc398c9d0a9bcf2b1bd4994399557985fe17bc216f0a40eb8eb49f29"
)
RETAINED_ARTIFACT_BYTES = 1_494_999

# Exactly `REGISTERED_INPUT` in tools/tg_a7_phala_tdx_workload.py; the producer
# compares the file literally, so any drift fails closed there too.
REGISTERED_INPUT = (
...
DSTACK_KEY_PATH = "sparkinterval/ch25-a7-boundary/enclave-signing-key/v1"
...
def note(message: str) -> None:
    print(f"ch25-a7-boundary phala-tdx prelude: {message}", flush=True)
...
def import_receipt_module():
    for candidate in ("/opt/sparkinterval", str(Path(__file__).resolve().parents[2])):
...
def run(args: argparse.Namespace) -> None:
    ...
    # ---- 8. The retained A.7 artifact ------------------------------------
    artifact_path = os.environ.get("TG_A7_ARTIFACT_PATH")
    if artifact_path:
        note(f"reading the retained A.7 artifact from {artifact_path}")
        artifact = read_local(
            Path(artifact_path), RETAINED_ARTIFACT_SHA256, "the A.7 artifact"
        )
    else:
        artifact_url = env("TG_A7_ARTIFACT_URL")
        note("fetching the retained A.7 artifact")
        artifact = fetch(
            artifact_url, RETAINED_ARTIFACT_SHA256, "the A.7 artifact"
        )
    if len(artifact) != RETAINED_ARTIFACT_BYTES:
        raise fail(
            f"the A.7 artifact is {len(artifact)} bytes, expected "
            f"{RETAINED_ARTIFACT_BYTES}"
        )
    write_exclusive(input_root / "a7_boundary.json", artifact, 0o444)

    # ---- 9. The registered input ----------------------------------------
    write_exclusive(
...
def main() -> int:
    try:
        ...
        print(
            f"ch25-a7-boundary phala-tdx prelude FAILED (unexpected): "
