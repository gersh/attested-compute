...
"""Print the per-integer leancompcert TDX evidence to stdout, to be retrieved.
...
* the campaign artifacts (ten freestanding executables, ~45 KB in total,
  pinned individually by SHA-256 inside `campaign-manifest.txt`, whose own
  digest is named inside `algorithmHash`) and the `dcap-qvl` binary (pinned by
  SHA-256) are not emitted: they are reproducible from their digests, which
  are.
...
EVIDENCE: tuple[tuple[str, str, bool], ...] = (
    ...
    # The campaign's own record: how many links ran, whether any returned a
    # `false` verdict, and the per-link exit statuses.  `campaign-run.json` is
    # what `registered-result.txt` summarises; `window-status.txt` is the
    # per-link detail behind it; `campaign-precheck.json` is the manifest
    # check the entry point ran before anything executed.
    ("output", "work/campaign-run.json", True),
    ("output", "work/window-status.txt", True),
    ("output", "work/campaign-precheck.json", True),
