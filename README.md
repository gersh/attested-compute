...
> **Azure was never run.** Every Azure row below describes a design that has
> not been executed on hardware: no `az` CLI, no subscription, no retained
> Azure evidence, and `attestation/verify_azure_ncc_evidence.py` fails at
> import. The Azure-only tests and tools were deleted on 2026-08-20; what
> remains is entangled with the H100 measured-worker machinery and is kept as
> specification. **The working attested path is Intel TDX** — see
> [`attestation/phala/README.md`](attestation/phala/README.md).
