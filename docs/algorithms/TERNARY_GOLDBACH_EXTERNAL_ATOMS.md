...
| `platt-little-mertens-stronger` | The same shared certificate checks the half-open range `[3, 7727068587)`; the upper endpoint is excluded because the closed statement is false there; `checked_little_stronger_real_of_local` proves the full real statement. This atom is another logical alias, not a second campaign. | No separate scan: replay the shared certificate. | Complete shared receipts, attach the registered local replay evidence, and identify the package-local step definition. |
...
The second upper endpoint is **exclusive**. The closed statement is false at
`7727068587`, where the sum is `5.6880854031502278e-06` against the majorant
`5.6880397241931255e-06` (relative excess `+8.03e-06`); a sweep of
`3 <= n <= 7727068587` finds that single violation. On `[3, 7727068586]` the
minimum relative margin is `1.47e-05`. Helfgott states the range with
`sum_(n<x)`, which is exactly this half-open `sum_(n<=x)` form, so this is a
...
stronger guard uses the right endpoint `n+1` throughout, with no closed
terminal case, because `7727068587` is excluded.
