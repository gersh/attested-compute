...
"""Build, run, and validate the bounded Goldbach warp-tail CUDA KAT."""
...
SOURCE = ROOT / "gpu/platform/h100/h100_tg_goldbach_warp_tail_kat.cu"
KIND = "sparkinterval.goldbach-warp-tail-kat.v1"


class KATError(RuntimeError):
    """The bounded CUDA cross-check failed."""
...
def validate_result(value: object) -> dict[str, object]:
    if not isinstance(value, dict):
        raise KATError("KAT result must be a JSON object")
    required = {
        "accepted",
        "compute_capability",
        "kind",
    ...
    if value["accepted"] is not True or value["kind"] != KIND:
        raise KATError("KAT did not report the exact accepted contract")
    if (
        ...
        raise KATError("KAT geometry or sieve cutoffs differ")
    if not isinstance(value["compute_capability"], str) or not re.fullmatch(
        r"[0-9]{1,2}\.[0-9]", value["compute_capability"]
    ):
        raise KATError("KAT compute capability is malformed")
    if (
        not isinstance(value["tail_prime_count"], int)
        or not isinstance(value["warp_prime_count"], int)
        or not 0 < value["warp_prime_count"] < value["tail_prime_count"]
    ):
        raise KATError("KAT prime partition is malformed")
    windows = value["windows"]
    if not isinstance(windows, list) or len(windows) != 4:
        raise KATError("KAT must report exactly four windows")
    previous = None
    for index, row in enumerate(windows):
        if not isinstance(row, dict) or set(row) != {
            ...
            raise KATError(f"KAT window {index} has wrong fields")
        if (
            ...
            raise KATError(f"KAT window {index} has wrong odd geometry")
        if not isinstance(row["fnv1a64"], str) or not re.fullmatch(
            r"[0-9a-f]{16}", row["fnv1a64"]
        ):
            raise KATError(f"KAT window {index} digest is malformed")
        if (
            not isinstance(row["set_bits"], int)
            or not 0 <= row["set_bits"] <= 1 << 18
        ):
            raise KATError(f"KAT window {index} bit count is malformed")
        if previous == (row["q_low"], row["fnv1a64"]):
            raise KATError("KAT contains a duplicate adjacent window")
        previous = (row["q_low"], row["fnv1a64"])
    if int(windows[-1]["q_high"]) != (1 << 64) - 1:
        raise KATError("final KAT window does not end at UINT64_MAX")
    return value
...
def main() -> int:
    ...
    if not SOURCE.is_file() or not arguments.nvcc.is_file():
        print("KAT source or nvcc is missing", file=sys.stderr)
        return 2
    try:
        with tempfile.TemporaryDirectory(prefix="tg-goldbach-warp-kat-") as temp:
            executable = Path(temp) / "goldbach-warp-tail-kat"
            build_argv = [
                str(arguments.nvcc),
                "-O3",
                "-std=c++20",
            ...
            report = {
                "accepted": True,
                "classification": "bounded-regression-not-production-evidence",
                "kind": "sparkinterval.goldbach-warp-tail-kat-report.v1",
                "arch": arguments.arch,
        ...
        encoded = (
            ...
            if arguments.pretty
            else json.dumps(report, separators=(",", ":"), sort_keys=True) + "\n"
        )
