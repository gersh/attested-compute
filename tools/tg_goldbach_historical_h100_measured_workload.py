...
"""Measured H100 worker for eight historical source-height Goldbach checkpoint leaves."""
...
INITIAL_DOMAIN = b"sparkinterval.measured-work-trace.goldbach-historical.h100.initial.v1\n"
STEP_DOMAIN = b"sparkinterval.measured-work-trace.goldbach-historical.h100.step.v1\n"
TREE_DOMAIN = b"sparkinterval/goldbach-historical-retained-tree/v1\0"
SHA256_RE = re.compile(r"[0-9a-f]{64}\Z")
MAX_ARCHIVE_BYTES = 64 * 1024 * 1024


class HistoricalGoldbachH100MeasuredWorkloadError(RuntimeError):
...
def _hex(value: Any, what: str) -> str:
    if not isinstance(value, str) or SHA256_RE.fullmatch(value) is None:
        raise HistoricalGoldbachH100MeasuredWorkloadError(
...
def _write_exclusive(path: Path, raw: bytes) -> None:
    ...
    try:
        ...
        while view:
            ...
            if count <= 0:
                raise HistoricalGoldbachH100MeasuredWorkloadError(f"short write: {path}")
            view = view[count:]
        os.fsync(descriptor)
...
def _exact_plan(
    ...
    if (
        ...
        raise HistoricalGoldbachH100MeasuredWorkloadError(
            "plan/source is not the exact source-height production profile"
        )
...
def _validate_input(args: argparse.Namespace) -> None:
    ...
    if not args.algorithm_id.startswith(GOLDBACH_H100_ALGORITHM_PREFIX):
        raise HistoricalGoldbachH100MeasuredWorkloadError("algorithm identity differs")
...
def _archive_path(args: argparse.Namespace) -> Path:
    return args.work / f"goldbach-historical-h100-group-{args.group_index:08d}.tar"
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.work.exists():
        raise HistoricalGoldbachH100MeasuredWorkloadError("work directory must be fresh")
    ...
    try:
        ...
        if retained_size > MAX_ARCHIVE_BYTES:
            raise HistoricalGoldbachH100MeasuredWorkloadError("retained export is too large")
        _write_exclusive(
            args.trace,
            canonical_json_bytes(
                _trace_value(
...
def verify_trace(args: argparse.Namespace) -> None:
    ...
    if retained_size > MAX_ARCHIVE_BYTES:
        raise HistoricalGoldbachH100MeasuredWorkloadError("retained export is too large")
    temporary = Path(
        tempfile.mkdtemp(prefix=".goldbach-historical-h100-replay-")
    )
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not 0 <= args.group_index < PRODUCTION_GROUPS:
            raise HistoricalGoldbachH100MeasuredWorkloadError("group index is outside the plan")
        if args.cuda_visible_device != 0:
            raise HistoricalGoldbachH100MeasuredWorkloadError("only CUDA device zero is reviewed")
        for name in (
        ...
        GoldbachGPUCampaignError,
        GoldbachBuildAdmissionError,
        HistoricalGoldbachH100MeasuredWorkloadError,
        OSError,
        ValueError,
    ) as error:
        print(f"historical Goldbach H100 measured workload error: {error}", file=sys.stderr)
        return 2
