...
"""Materialize one source-reviewed lowered-Goldbach Azure H100 group."""
...
SCHEMA_VERSION = 1
SITE_KIND = "sparkinterval.azure.h100.goldbach10pow27-materializer-site.v1"
MANIFEST_KIND = "sparkinterval.azure.h100.goldbach10pow27-materialization.v1"
SITE_FIELDS = {
...
PLAN_GROUP = (
    "ternary-goldbach-finite-below-10pow27-v1::create-lowered-binary-plan"
)
BUILD_ENVIRONMENT = H100_BUILD_ENVIRONMENT


class Goldbach10Pow27H100MaterializerError(RuntimeError):
...
def _exact(value: Any, fields: set[str], what: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != fields:
        ...
        raise Goldbach10Pow27H100MaterializerError(
...
def load_site(
    ...
    try:
        ...
        raise Goldbach10Pow27H100MaterializerError(str(error)) from error
    source = _directory(
...
def _copy_tree(source: Path, destination: Path) -> None:
    if destination.exists() or source.is_symlink() or not source.is_dir():
        raise Goldbach10Pow27H100MaterializerError("unsafe source-tree copy")
...
def _plan_predecessor(
    ...
    if record is None:
        raise Goldbach10Pow27H100MaterializerError(
            "lowered binary plan predecessor has no portfolio receipt"
        )
    azure_portfolio._validate_task_record(context, task_id, record)
    if record["stage"] != "verified_receipt_recorded":
        raise Goldbach10Pow27H100MaterializerError(
            "lowered binary plan predecessor is incomplete"
        )
    try:
    ...
    result = _validate_cpu_result(
        receipt, "create-lowered-binary-plan", 0, export
    )
    manifest = verify_retained_export_archive(
        export, "create-lowered-binary-plan", 0
    )
...
def plan_materialization(
    ...
    if factory is None or shard.get("argv") != list(factory.portfolio_argv):
        raise Goldbach10Pow27H100MaterializerError(
            "portfolio shard has no exact lowered-Goldbach H100 factory"
        )
    ...
    return {
        ...
        "classification": (
            "reviewed_goldbach10pow27_h100_materialization_plan_not_execution_evidence"
        ),
        ...
        "retained_export_relative_path": (
            f"bundle-root/work/goldbach10pow27-h100-{shard_index:08d}/"
            f"goldbach10pow27-h100-group-{shard_index:08d}.tar"
        ),
...
def _extract_plan(export: Path, destination: Path) -> Any:
    temporary = Path(tempfile.mkdtemp(prefix=".goldbach10pow27-h100-plan-"))
    try:
    ...
    if (
        ...
        raise Goldbach10Pow27H100MaterializerError(
            "predecessor plan is not the exact lowered profile"
        )
    return plan
...
def _runtime_closure(
    context: azure_portfolio.PortfolioContext,
    site: Mapping[str, Any],
    factory: Goldbach10Pow27H100WorkloadFactory,
    artifact_root: Path,
    ...
    if verify_hardened_source_tree(source_copy) != source_identity:
        raise Goldbach10Pow27H100MaterializerError("copied hardened source differs")
    ...
    if plan.executable_sha256 != admission.core["executable"]["sha256"]:
        raise Goldbach10Pow27H100MaterializerError(
            "lowered binary plan does not bind the admitted executable"
        )
    ...
    source_manifest = {
        ...
        "kind": "sparkinterval.goldbach10pow27-h100-source-closure.v1",
    ...
    records = [
        ...
        _artifact_record(
            plan_path, artifact_root, role="signed_predecessor_lowered_binary_plan",
...
def _job(
    ...
    if algorithm_hash not in factory.algorithm_id:
        raise Goldbach10Pow27H100MaterializerError("algorithm ID/hash binding differs")
    ...
    job = {
        ...
        "job_id": f"tg-goldbach10pow27-h100-{factory.shard_index:08d}-v1",
...
def _operator_config(
    site: Mapping[str, Any], factory: Goldbach10Pow27H100WorkloadFactory,
    ...
    worker = base_worker / "goldbach10pow27" / f"group-{factory.shard_index:08d}"
    ...
    template["lean_review"] = {
        "namespace": "Goldbach10Pow27Operational",
        "registered_invocation": None,
    }
    template["outputs"] = {
...
def materialize(
    ...
    if factory is None:
        raise Goldbach10Pow27H100MaterializerError("reviewed factory disappeared")
    predecessor_export, predecessor = _plan_predecessor(context, site)
    ...
    try:
        ...
        manifest = {
            ...
            "classification": (
                "source_reviewed_goldbach10pow27_h100_operator_validated_"
        ...
        raise Goldbach10Pow27H100MaterializerError(
            f"H100 lowered-Goldbach materialization failed closed: {error}"
        ) from error
    finally:
        if stage.exists():
...
def export_retained_group(
    ...
    temporary = Path(tempfile.mkdtemp(prefix=".goldbach10pow27-h100-export-review-"))
    try:
...
__all__ = [
    "Goldbach10Pow27H100MaterializerError", "export_retained_group", "load_site",
