...
"""Closed H100 factory for one source-height historical Goldbach worker group.
...
PHASE_ID = "h100-8192-groups-of-eight-checkpoint-leaves"
GROUP_ID = f"{CAMPAIGN_ID}::{PHASE_ID}"
SHARD_COUNT = 8_192
PHASE_DEPENDENCIES = (f"{CAMPAIGN_ID}::create-production-plan",)
...
TRACE_DEFINITION = (
    "sparkinterval.challenge-work-trace.goldbach-historical-h100-group.v1\n"
...
def algorithm_definition(
    ...
    if not 0 <= group_index < SHARD_COUNT:
        raise ValueError("historical H100 group index is outside the reviewed plan")
    ...
    return (
        ...
        "semantics=run-eight-strided-source-height-checkpoint-leaves-and-retain-receipts\n"
...
def domain_value(group_index: int) -> dict[str, Any]:
    return {
        ...
        "even_limit_inclusive": PRODUCTION_EVEN_LIMIT,
        "even_start_inclusive": PRODUCTION_EVEN_START,
        "group_index": group_index,
    }
...
# Retain the concise consumer-facing name used by the terminal verifier.
expected_claim_identity = h100_expected_claim_identity


@dataclass(frozen=True)
class HistoricalGoldbachH100WorkloadFactory:
    factory_id: str
    phase_id: str
    group_id: str
    shard_index: int
...
def _measured_argv(mode: str, group_index: int, algorithm_id: str) -> tuple[str, ...]:
    return (
        "artifacts/python3", "-I",
        "tools/tg_goldbach_historical_h100_measured_workload.py", mode,
        ...
        "--output", "@output@",
        "--trace", "@trace@",
        "--work", f"work/goldbach-historical-h100-{group_index:08d}",
...
def _projection_job(
    ...
    return {
        ...
        "job_id": f"tg-goldbach-historical-h100-{group_index:08d}-v1",
...
def make_factory(
    ...
    definition = algorithm_definition(group_index, admission)
    return HistoricalGoldbachH100WorkloadFactory(
        factory_id=f"historical_goldbach_h100_group_{group_index:08d}_v1",
        phase_id=PHASE_ID,
        group_id=GROUP_ID,
...
) -> HistoricalGoldbachH100WorkloadFactory | None:
...
__all__ = [
    ...
    "expected_claim_identity", "expected_execution_projection_sha256",
